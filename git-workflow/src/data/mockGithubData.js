...
const SEED_PR_FILES = {
  "granjur/ubs-server#39": [
    {
      filename: "src/routes/guests.js",
      status: "modified",
      additions: 18,
      deletions: 2,
    },
    {
      filename: "src/middleware/rateLimit.js",
      status: "added",
      additions: 24,
      deletions: 0,
    },
  ...
  "granjur/badar-hms#13": [
    {
      filename: "src/services/housekeeping.js",
      status: "modified",
      additions: 35,
      deletions: 0,
    },
    {
      filename: "src/routes/reports.js",
      status: "modified",
      additions: 28,
      deletions: 1,
    },
  ...
  "Aashir-Adnan/UBS-Doc#21": [
    {
      filename: "docs/intro/backend-setup.md",
      status: "added",
      additions: 86,
      deletions: 0,
    },
...
export async function mockGhFetch(path, opts = {}) {
  ...
  if (m) {
    ...
    const state = m[3];
    if (state === "open")
      return all.filter((i) => i.state === "open" && !i.pull_request);
    if (state === "closed")
      return all.filter((i) => i.state === "closed" && !i.pull_request);
